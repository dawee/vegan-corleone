...
actions.setNewPosition =  Dispersive.createAction((lat, lng) => ({lat, lng}));
...
RestaurantStore.bindAction(actions.setNewPosition, (location) => {
  RestaurantStore.location = location;
  RestaurantStore.trigger('change');
});
...
class GoogleMapElement extends GoogleMap {
  componentDidMount () {
    setTimeout(() => actions.setRestaurantsStoreReady(), 0);
  }
}
...
class RestaurantMap extends Corleone.Component {
  ...
  onCenterChanged() {
    actions.setNewPosition(
      this.refs.map.getCenter().lat(),
      this.refs.map.getCenter().lng()
    );
  }
